function [A,B,S,C,C_tilde,cost,valid] = BTD_Var_main(SRI,HSI,MSI,P1,P2,Pm,R,B,C,C_tilde,Niter,lambda)
...
%     end
    ...
    valid(m) = norm(tens2mat(SRI,[],3) - pw_vec(A,B,R)*C','fro');
    diff_valid(m) = valid(m-1) - valid(m);
    
    fprintf('Iter: %d/%d, Err: %.4f (%.4f)...\n', m, Niter, valid(m), valid(m-1)-valid(m));
